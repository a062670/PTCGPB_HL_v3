...
class CryptionClient {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      this.socket.on("getPlayerIds_response", (response) => {
        this.handleResponse("getPlayerIds", response);
      });

      this.socket.on("getGodPackList_response", (response) => {
        this.handleResponse("getGodPackList", response);
      });
    });
  }
  ...
  getGodPackList(lastGetAt) {
    return new Promise((resolve, reject) => {
      ...
      this.socket.emit("getGodPackList", {
        requestId,
        lastGetAt,
      });
    });
...
const getPlayerIds = async (max, friendId) => {
  try {
    if (!cryptionClient.socket) {
      await cryptionClient.connect();
    }
    return await cryptionClient.getPlayerIds(max, friendId);
  } catch (error) {
    console.error("getPlayerIds error:", {
      message: error.message,
    });
  }
  return [];
...
const getGodPackList = async (lastGetAt) => {
  try {
    if (!cryptionClient.socket) {
      await cryptionClient.connect();
    }
    return await cryptionClient.getGodPackList(lastGetAt);
  } catch (error) {
    console.error("getGodPackList error:", {
      message: error.message,
    });
  }
  return [];
